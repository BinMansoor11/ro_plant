...
const TabScreen = (props) => {
  const { emptyCard, card, data, length, i, setRefresh } = props;
                ...
                <SecondaryButton
                  ...
                  onPress={() => (
                    ...
                    i == 1 &&
                      (validation(),
                      Linking.openURL(
                        `sms:${data.phone_number}?body=${'hey there...'}`,
                      ))
                  )}
                />
              );
...
const HomeScreen = (props) => {
  ...
  const [dailyDelivery, setDailyDelivery] = useState({ today: [] });
  const { today } = dailyDelivery;
  ...
  return (
    <>
      <GlobalHeader
        headingText="Today Deliveries"
        boy
        navigation={props.navigation}
        rightButton
        rightButtonText={'LOGOUT'}
        rightButtonPress={() => navigation.navigate('GetStarted')}
      />
